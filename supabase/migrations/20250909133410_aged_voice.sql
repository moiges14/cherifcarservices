/*
  # Mise à niveau du système de réservation

  1. Nouvelles colonnes pour la table bookings
    - `passengers` (integer) - Nombre de passagers
    - `special_requests` (text) - Demandes spéciales
    - `contact_phone` (text) - Téléphone de contact
    - `estimated_price` (numeric) - Prix estimé
    - `estimated_distance` (numeric) - Distance estimée
    ...
    - `rating` (integer) - Note donnée par le client
    - `review_comment` (text) - Commentaire de l'avis

  2. Nouvelles tables
    - `booking_notifications` - Notifications pour les réservations
    - `driver_locations` - Localisation en temps réel des chauffeurs
...
BEGIN
  ...
  CASE NEW.status
    ...
    WHEN 'completed' THEN
      ...
      notification_message := 'Votre course ' || NEW.booking_reference || ' est terminée. N\'oubliez pas de noter votre chauffeur !';
    WHEN 'cancelled' THEN
